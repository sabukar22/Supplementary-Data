...
input_file <- "JSQH194_tracks_6.csv"
input_directory <- ""
output_directory <- ""
...
for (track_id in names(data_split)) {
  file_name <- paste0("Track_", track_id, ".xlsx")
  write.xlsx(data_split[[track_id]], file_name)
}

file_list <- list.files(pattern = "^Track_\\d+\\.xlsx$") 
...
process_file <- function(file_path) {
  tryCatch({
    ...
    output_file <- paste0("Tree_", file_number, ".tre")
...
file_list <- list.files(input_directory, pattern = "Track_\\d+_tree_length\\.xlsx", full.names = TRUE)
...
tree_files <- list.files(pattern = "Tree_.*\\.tre$")
gfp_files <- list.files(pattern = "Track_.*_GFP\\.xlsx$")
...
for (i in seq_along(tree_files)) {
  ...
  gfp_file <- paste0("Track_", track_number, "_GFP.xlsx")
  ...
  if (file.exists(tree_files[i]) && file.exists(gfp_file)) {
    ...
    write.csv(species_df, file = paste0(track_number, "_T.csv"), row.names = FALSE)  # No row names
    write.csv(nodes_df, file = paste0(track_number, "_N.csv"), row.names = FALSE)  # No row names
  }
}
...
for (tree_file in tree_files) {
  ...
  trait_T_file <- paste0(track_number, "_T.csv")
  trait_N_file <- paste0(track_number, "_N.csv")
...
ggsave('Combined_Trait_Trees.svg', combined_plot, bg = 'transparent', width = 25, height = 5 * length(plots), limitsize = FALSE)

cat("Processing complete. Combined plot saved as 'Combined_Trait_Trees.svg'.\n")
